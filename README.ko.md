...
- Windows 10/11 지원. `winget install Codex -s msstore`로 Codex를 설치한 뒤 PowerShell에서 `rds codex install`을 실행하면 됩니다. 설치 시 `%USERPROFILE%\.rds\bin\` 아래에 `rds.cmd` (PowerShell/cmd용)과 `rds` (Git Bash/WSL용) wrapper를 동시에 생성하며, 스킬은 `%USERPROFILE%\.agents\skills\`에 배치됩니다.
