...
설치 후 setup workflow를 실행하면 RDS는 짧은 onboarding 질문만 묻습니다: 연구 분야, 프로젝트 주제, 프로젝트 scaffold, linking mode, glossary 생성 여부, log 말투, 현재 연구자 과정. setup 대화에서 사용자의 언어를 감지해 이후 discussion log의 기본 작성 언어로 사용합니다.

그 답변을 바탕으로 context 파일, discussion log, literature note, metadata, Claude/Codex/Gemini용 instruction 파일이 들어 있는 RDS Project를 만듭니다. 사용자 설정은 `00_context/user_profile.md` 한 곳에 모아 쉽게 수정할 수 있습니다. 현재 막힌 지점, 데이터 형식, 외부 도구, 세부 협업 선호는 이후 일반 discussion 중 자연스럽게 파악합니다.
...
참고:
- 설치된 skill 파일은 `~/.agents/skills/rds-*`에 배치됩니다 (Codex 공식 사용자 스킬 경로).
- 설치 과정에서 안정적인 로컬 backend wrapper를 생성하므로, 환경별 PATH 상속 차이가 있어도 동작이 일관됩니다.
- Windows 10/11 네이티브 지원. `winget install Codex -s msstore`로 Codex를 설치한 뒤 PowerShell에서 `rds codex install`을 실행합니다. 설치 시 `%USERPROFILE%\.rds\bin\` 아래에 `rds.cmd` (PowerShell/cmd용)와 `rds` (Git Bash용) wrapper를 동시에 생성하며, 스킬은 `%USERPROFILE%\.agents\skills\`에 배치됩니다. WSL에서 Codex를 사용하는 경우에는 Windows 설치를 공유하지 말고, WSL 환경 안에서 `rds codex install`을 별도로 실행하세요.
...
```text
RDS Project
├── 00_context/        # 사용자 설정, 프로젝트 정체성, 결정, assumption, open question
