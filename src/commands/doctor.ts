...
export function detectMode(root: string): "repo" | "project" | "unknown" {
  if (
    exists(path.join(root, ".claude-plugin", "plugin.json")) &&
...
function existsCheck(root: string, name: string, rels: string[], fix = ""): Check {
...
function jsonManifestCheck(root: string, rel: string, name: string, fix: string): Check {
  const filePath = path.join(root, rel);
  if (!exists(filePath)) {
    return { name, ok: false, detail: `missing ${rel}`, fix };
  }
  try {
    const data = JSON.parse(readText(filePath)) as Record<string, unknown>;
    const missing = ["name", "version", "description", "skills", "license", "repository"].filter(
      (key) => !(key in data),
    );
    return {
      name,
...
function codexMarketplaceCheck(root: string): Check {
  ...
  try {
    const data = JSON.parse(readText(marketplace)) as { plugins?: unknown };
    const plugins = Array.isArray(data.plugins) ? data.plugins : [];
    const hasRds = plugins.some((plugin) => {
      return (
        typeof plugin === "object" &&
        plugin !== null &&
        (plugin as { name?: unknown }).name === "rds"
      );
    });
    if (!hasRds) {
      return {
...
function claudeMarketplaceCheck(root: string): Check {
...
function backendCheck(root: string, mode: string): Check {
  if (mode !== "repo") {
...
function repoChecks(root: string): Check[] {
  return [
    nodeCheck(),
    ...
    existsCheck(
      root,
      "TypeScript backend",
      ["package.json", "tsconfig.json", "src/cli.ts", "dist/cli.js"],
      "Run npm run build",
    ),
    existsCheck(
      root,
...
function projectChecks(root: string): Check[] {
  ...
  const indexMentionsDiscussions =
    exists(discussionIndex) && readText(discussionIndex).includes("Discussion Index");
  ...
  return [
    ...
    existsCheck(
      root,
      "RDS Project structure",
      ["00_context", "04_discussions", "05_literature", "99_meta"],
      "Run RDS setup or restore fixed folders",
    ),
    existsCheck(
      root,
      "Agent instructions",
      ["CLAUDE.md", "AGENTS.md", "GEMINI.md"],
      "Regenerate agent adapters",
    ),
    existsCheck(
      root,
      "Runtime config",
      ["99_meta/scaffold_decisions.md"],
      "Regenerate scaffold decisions",
    ),
    {
      name: "Linking mode",
      ok: validLinking,
...
function renderTable(checks: Check[]): void {
...
export function doctorCommand(argv: string[], cwd = process.cwd()): number {
