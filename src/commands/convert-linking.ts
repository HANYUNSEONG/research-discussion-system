...
function markdownTargetToWiki(linkPath: string, sourceFile?: string, project?: string): string {
...
function wikiTargetToMarkdown(linkPath: string, sourceFile?: string, project?: string): string {
...
export function toObsidian(text: string, sourceFile?: string, project?: string): string {
  return text.replace(MD_LINK, (_match, label: string, linkPath: string, anchor?: string) => {
    const clean = markdownTargetToWiki(linkPath, sourceFile, project);
    return `[[${clean}${anchor ?? ""}|${label}]]`;
  });
}

export function toPlain(text: string, sourceFile?: string, project?: string): string {
  return text.replace(WIKI_LINK, (_match, linkPath: string, anchor?: string, label?: string) => {
...
function updateConfig(project: string, mode: string): void {
...
export function convertLinkingCommand(argv: string[], cwd = process.cwd()): number {
  const args = parseArgs(argv);
  const targetArg = readString(args, "target", true);
  const mode = readChoice(args, "to", LINKING_CHOICES);
  const project = resolvePath(targetArg as string, cwd);
