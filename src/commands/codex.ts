...
function codexHome(homeArg: string | undefined): string {
  if (homeArg) {
...
function writeWrapper(wrapperPath: string, supportRoot: string): void {
...
function generateSkill(skillName: string, wrapperPath: string, supportRoot: string): string {
  return `---
name: ${skillName}
...
function readManifest(home: string): InstallManifest | null {
  ...
  try {
    const parsed = JSON.parse(readText(manifestFile)) as InstallManifest;
    if (
    ...
    if (smokeTest && exists(manifest.wrapperPath)) {
      ...
      try {
        const run = (args: string[]): void => {
          ...
          if (result.status !== 0) {
            fail(
              `Smoke test failed: ${[manifest.wrapperPath, ...args].join(" ")}\n${
