...
const SUPPORT_DIRS = ["dist", "core", "templates", "scaffolds", "adapters", "skills"] as const;
...
function packageVersion(repoRoot: string): string {
  ...
  const pkg = JSON.parse(readText(pkgPath)) as { version?: unknown };
...
function userHome(homeArg: string | undefined): string {
  if (homeArg) {
...
function rdsDataRoot(home: string): string {
  return path.join(home, ".rds", "codex");
}

function installManifestPath(home: string): string {
  return path.join(rdsDataRoot(home), "install-manifest.json");
}

function ensureExists(rel: string, repoRoot: string): string {
...
interface WrapperPaths {
  primary: string;
  posix?: string;
  windows?: string;
}

function wrapperPaths(home: string): WrapperPaths {
...
function writeWrappers(paths: WrapperPaths, supportRoot: string): string[] {
...
function readManifest(home: string): InstallManifest | null {
  ...
  try {
    const parsed = JSON.parse(readText(manifestFile)) as Partial<InstallManifest>;
    if (
    ...
    const wrappers = Array.isArray(parsed.wrapperPaths)
      ? parsed.wrapperPaths.filter((entry): entry is string => typeof entry === "string")
      : [parsed.wrapperPath];
    return {
      ...
      skillNames: parsed.skillNames as string[],
      platform: (parsed.platform as NodeJS.Platform | undefined) ?? process.platform,
    };
  } catch {
    return null;
  }
...
function writeManifest(home: string, manifest: InstallManifest): void {
  writeText(installManifestPath(home), `${JSON.stringify(manifest, null, 2)}\n`);
}

function installCodex(repoRoot: string, home: string, force: boolean): number {
...
function uninstallCodex(home: string): number {
...
function wrapperIsUsable(wrapperPath: string, isPrimary: boolean): boolean {
  if (!exists(wrapperPath)) return false;
  if (!isPrimary) return true;
  if (IS_WINDOWS) return true;
  return isExecutable(wrapperPath);
}

function codexDoctor(home: string, smokeTest: boolean): number {
  const manifest = readManifest(home);
    ...
    if (smokeTest && exists(manifest.supportRoot)) {
      ...
      try {
        const run = (args: string[]): void => {
          ...
          if (result.status !== 0) {
            fail(
              `Smoke test failed: node ${[cliPath, ...args].join(" ")}\n${
...
export function codexCommand(argv: string[], repoRoot: string): number {
