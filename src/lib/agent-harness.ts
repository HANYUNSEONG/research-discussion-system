...
export const EXPECTED_CLAUDE_SESSION_COMMAND =
...
export const EXPECTED_CODEX_SESSION_COMMAND =
...
function readJsonFile(root: string, rel: string): unknown {
...
function extractSharedContractLines(text: string): string[] | null {
...
function sharedContractProblems(label: string, text: string): string[] {
  ...
  for (const line of lines) {
    if (!expected.includes(line as (typeof EXPECTED_SHARED_CONTRACT_LINES)[number])) {
...
function commandHooks(config: unknown, eventName: string): CommandHook[] {
...
function denyListIncludes(config: unknown, permission: string): boolean {
  const permissions = asRecord(asRecord(config)?.permissions);
  const deny = permissions?.deny;
  return Array.isArray(deny) && deny.includes(permission);
}

function exactlyOneCommand(commands: CommandHook[], expectedCommand: string): boolean {
  return commands.length === 1 && commands[0].command === expectedCommand;
}

export function checkAdapterContractDrift(root: string): string[] {
  const problems: string[] = [];
  ...
  for (const contractFile of RDS_ADAPTER_CONTRACT_FILES) {
    ...
    for (const requiredContext of REQUIRED_CONTEXT_REFERENCES) {
      if (!text.includes(requiredContext)) {
        problems.push(
          `${contractFile.label} does not mention required context: ${requiredContext}`,
        );
      }
    }
  }
...
export function checkRuntimeHarnessConfig(root: string): string[] {
  const problems: string[] = [];
  let claudeSettings: unknown = null;
  let codexHooks: unknown = null;
  ...
  if (!claudeSettings) {
    ...
    if (!exactlyOneCommand(commands, EXPECTED_CLAUDE_SESSION_COMMAND)) {
      problems.push(
        ".claude/settings.json SessionStart command does not match the approved harness message",
      );
    }
  }
  ...
  if (!codexHooks) {
    ...
    if (!exactlyOneCommand(commands, EXPECTED_CODEX_SESSION_COMMAND)) {
      problems.push(
        ".codex/hooks.json SessionStart command does not match the approved harness message",
      );
    } else {
      const command = commands[0];
      if (command.statusMessage !== EXPECTED_CODEX_STATUS_MESSAGE) {
        problems.push(
          ".codex/hooks.json SessionStart statusMessage does not match the approved value",
        );
      }
...
export function checkRootAgentInstructions(root: string): string[] {
  const agentFiles = ["AGENTS.md", "CLAUDE.md"] as const;
